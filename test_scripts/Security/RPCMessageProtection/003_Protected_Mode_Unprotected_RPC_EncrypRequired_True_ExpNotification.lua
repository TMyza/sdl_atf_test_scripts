...
-- 3) RPC needs protection, encryption_required parameters to App within app_policies = true or (nil) and to the
...
--[[Local Function]]
local function unprotectedRpcInProtectedModeEncryptedRequired()
	local params = {
    cmdID = 1,
    menuParams = {
      position = 1,
...
for _, tc in common.spairs(testCases) do
  ...
  runner.Step("Unprotected RPC in protected mode, param for App="..tostring(tc.a)..",for Gruop="..tostring(tc.f),
    unprotectedRpcInProtectedModeEncryptedRequired)
